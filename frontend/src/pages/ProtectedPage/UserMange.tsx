...
interface EditAvatarFormValues {
  avatarUrl: {
    url: string
    public_id: string
  }[]
...
function UserMange() {
  ...
  const editAvatarFormRef = useRef<ProFormInstance>(null)
  ...
  const handleEditAvatar = async (idUser: string, value: EditAvatarFormValues, publicId: string) => {
    const { avatarUrl } = value
    const { url, public_id } = avatarUrl[0]
    await uploadService.deleteImage(publicId)
    await userService.editUsers(idUser, { avatarUrl: url, avatarId: public_id })
    setOpenUploadImageId(null)
    formRef.current?.resetFields()
    actionRef.current?.reload()
  }
  ...
  const columns: ProColumns<User>[] = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Flex gap={8} align='center'>
          ...
          />
          <Button style={{ padding: 3 }} type='link' onClick={() => setOpenUploadImageId(record._id)}>
            <EditOutlined />
          </Button>
          <Modal
            title='Đổi ảnh đại diện'
            open={openUploadImageId === record._id}
            onCancel={() => setOpenUploadImageId(null)}
            destroyOnClose
            footer={null}
          >
            <ProForm
              formRef={editAvatarFormRef}
              onFinish={(value: EditAvatarFormValues) => handleEditAvatar(record._id, value, record.avatarId)}
              submitter={{
                searchConfig: {
                  submitText: 'Cập nhập',
                  resetText: 'Hủy'
    ...
    },
    {
      ...
      search: false,
      ellipsis: true,
      tooltip: 'Mật khẩu của người dùng',
      render: () => <p>**********</p>
    },
    {
      title: 'Vai trò',
      ...
      <ProTable
        ...
        editable={{
          ...
          onSave: async (_, record) => (
            await userService.editUsers(record._id, {
